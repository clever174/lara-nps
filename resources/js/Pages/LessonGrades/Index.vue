            ...
            <div class="bg-white rounded-lg p-1">
                ...
                >
                    <Column expander style="width: 35%">
                        <template #header>
                            <span class="flex-1 text-center font-bold">Тренинг</span>
                        </template>
                        <template #body="{ node }">
                            <div class="w-full text-left">
                <span v-if="node.children?.length">
                  {{ node.data.course_title }}
                </span>
                                <span v-else>
                  {{ node.data.lesson_title }}
                </span>
                            </div>
                        </template>
                    </Column>
                        ...
                        <template #body="{ node }">
                            <div class="flex justify-center w-full gap-2">
                                <Link
                                    ...
                                    :key="i"
                                    :href="route('lesson-grades.list', getRatingRoute(node, i))"
                                    class="no-underline"
                                >
                                    <Tag
                                        :value="node.data['rating_' + i]"
                                        :title="'Оценка ' + i"
                    ...
                    <Column style="width: 10%">
                        ...
                        <template #body="{ node }">
                            <div class="w-full text-center">
                                <Link
                                    :href="route('lesson-grades.list', getDetailsRoute(node))"
